...
namespace selective_goal_checker
{
    ...
    nav2_util::declare_parameter_if_not_declared(
        node,
        plugin_name + ".y_tolerance_when_check_x_only", rclcpp::ParameterValue(0.2));
    nav2_util::declare_parameter_if_not_declared(
        node,
    ...
    node->get_parameter(plugin_name + ".y_tolerance_when_check_x_only", y_tolerance_);
    ...
    const geometry_msgs::msg::Twist & velocity)
    {
        if (check_xy_ && check_xy_dist_) {
            ...
            double dy = abs(T_robot_goal(1,3));
            // std::cout << "dx: " << dx << std::endl;
            if (!(dx < xy_goal_tolerance_ && dy < y_tolerance_)) {
                return false;
            }
        }
        if(check_v_){
            // TODO: add velocity check code here.
            double v_xy = std::hypot(velocity.linear.x,velocity.linear.y);
            if(!(v_xy<0.005 && velocity.angular.z<0.01)){
                std::cout << "vxy: " << v_xy << " vyaw: " <<  velocity.angular.z << std::endl;
                return false;
            }
        }
    ...
    for (auto & parameter : parameters) {
        ...
        if (type == ParameterType::PARAMETER_DOUBLE) {
            ...
            } else if (name == plugin_name_ + ".y_tolerance_when_check_x_only") {
                y_tolerance_ = parameter.as_double();
            }
        } else if (type == ParameterType::PARAMETER_BOOL) {
            if (name == plugin_name_ + ".stateful") {
                stateful_ = parameter.as_bool();
            } else if (name == plugin_name_ + ".check_xy_dist") {
                check_xy_dist_ = parameter.as_bool();
            } else if (name == plugin_name_ + ".check_x_only") {
                check_x_ = parameter.as_bool();
            } else if (name == plugin_name_ + ".check_v") {
                check_v_ = parameter.as_bool();
            }
        }
    }
